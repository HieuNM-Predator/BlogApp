  ...
  <v-container>
    <v-card>
      <v-form ref="form" @submit.prevent="onSubmit()">
        <v-card-title>
          <span class="text-h5">Edit Blog Post</span>
        </v-card-title>
...
const getBlogPostById = () => {
  axios
    .get(`https://localhost:7200/api/BlogPosts/${route.params.blogPostId}`)
    .then((response) => {
...
const editBlogPost = (blogPostId, params) => {
  axios
    .put(`https://localhost:7200/api/BlogPosts/${blogPostId}`, { ...params })
    .then((response) => {
      console.log(response.data);
      // showSnackbar("Edit blog post successfully", "success");
      // getAllBlogPosts();
    })
    .catch((err) => {
